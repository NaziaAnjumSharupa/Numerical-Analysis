...
using namespace std;
float squr(float x,float a,float b, float c)
    {
    return a*x*x+b*x+c;
    }
...
int main()
{
    ...
    cin>>a>>b>>c>>d;
        ...
        cout<<"Invalid input!";
    f1=n==2?squr(x1,a,b,c):cube(x1,a,b,c,d);
    f2=n==2?squr(x2,a,b,c):cube(x2,a,b,c,d);
    if(f1*f2>0)
    {
    ...
    {
        x0=(x1+x2)/2;
        f0=n==2?squr(x0,a,b,c):cube(x0,a,b,c,d);
        if(f1*f0<0)
            x2=x0;
        else
        {
